...
void playFreq(SndCmdIDType cmd, int freq, int time)
{
   ...
   if (keepEupEPrefs.state.sound != 0){
      ...
      keepEupEPrefs.freq = soundCmd.param1 = freq; /*freq in hz */
      keepEupEPrefs.dur = soundCmd.param2 = time; /*max dur in ms */
      soundCmd.param3 = keepEupEPrefs.amp; /*amp 0 - sndMaxAmp */
