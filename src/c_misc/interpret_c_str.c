...
#define START_OF_ERROR_CASES 21
#define END_OF_ERROR_CASES 25

int main(int argv, char **argc) 
{
  char *s[END_OF_ERROR_CASES+1][3] = {
      { "\\\"", "\"", "", },
      { "abcd efgh", "abcd efgh", "",},
      { "abcd efgh\\r\\n", "abcd efgh\r\n", "",},
      ...
      // multiple-quote cases - revealed a problem (now fixed) when integrated into tbx logic:
      { "escape(\"\",\"\\\"\",\"\\\"\")", "escape(\"\",\"\"\",\"\"\")", ""},
      { "escape(\"\\\"\",\"\\\"\",\"\\\"\")", "escape(\"\"\",\"\"\",\"\"\")", ""},
      { "escape(\"\\\"\\\"\",\"\\\"\",\"\\\"\")", "escape(\"\"\"\",\"\"\",\"\"\")", ""},
      { "escape(\"\\\"\\\"\\\"\\\"\\\"\\\"\\\"\\\"\\\"\\\"\",\"\\\"\",\"\\\"\")", "escape(\"\"\"\"\"\"\"\"\"\"\"\",\"\"\",\"\"\")", ""},
...
*/
  ...
  for(i=0;i<START_OF_ERROR_CASES;i++) {
    printf("%50s ### %s\n",s[i][1],s[i][0]);
  }
  ...
  for(i=START_OF_ERROR_CASES;i<=END_OF_ERROR_CASES;i++) {
      char *si = interpret_c_str(s[i][0],&rv);
      if (rv!=0 && s[i][1][0] != 0) {
