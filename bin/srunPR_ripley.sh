...
LOG=srunPR_rip_${DTS}.log

source srunPRHOSTS.sh

for h in $AR_HOSTS ; do
   echo h=$h;
   ssh omn@$h -i ${SSHPR} 'bin/bci -listals; for f in /data/dfl-dir/ripley/*; do echo f=$f $(cat $f); done
                           echo SBUG; bin/clex -ch 1 -s -1d'
done 2>&1 |tee $LOG
...
for h in $AR_HOSTS $C_SMSC_HOSTS $M_SMSC_HOSTS $C_CARE_HOSTS $M_CARE_HOSTS; do
   ...
   #ssh omn@$h -i ${SSHPR} 'bin/bci -listals; ls -alstr *core*; ls -alstr ripley*; cat ripleyIDs.log'
   ssh omn@$h -i ${SSHPR} 'bin/bci -listals; for f in /data/dfl-dir/ripley/*; do echo f=$f $(cat $f); done'
