...
FILES=$(ls -tr *-iostat)
[[ -z "$FILES" ]] && echo "error: no files found" && exit
#test -s *-iostat || echo "error: no files found" && exit

# DEVICES
DEVICES=$(cat *-iostat |head -150 |awk '{print $1}' |grep -P "^(hd|sd|sc|dm)"|sort|uniq)
