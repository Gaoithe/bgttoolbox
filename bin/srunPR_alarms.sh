...
for h in $AR_HOSTS $C_SMSC_HOSTS $M_SMSC_HOSTS $C_CARE_HOSTS $M_CARE_HOSTS; do
   ...
   ssh omn@$h -i ${SSHPR} 'bin/bci -listals; bin/clex -ch 2 -s '"$P"' |grep -P "RAISE|CLEAR|Availab"'
   #ssh omn@$h -i ~/.ssh/proximus2 'bin/bci -listals; bin/clex -ch 2 -s '"$P"' |grep -P "RAISE|CLEAR|Availab"'
   #ssh omn@$h ${SSHPR} 'echo h=$h; ls -alstr *core*; bin/bci -listals; bin/clex -ch 2 -s '"$P"' |grep Availab'
done 2>&1 |tee $LOG
