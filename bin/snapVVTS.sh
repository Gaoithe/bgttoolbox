...
DTS=$(date +%Y%m%d_%H%M)
#LOG=snapLOG/snapTS_VantrixVersion_${DTS}.log
LOG=snapLOG/snapTS_cstat_${DTS}.log
date |tee -a ${LOG}
...
for h in $IPKOHOSTS $TSHOSTS; do
 ...
 ssh omn@$h "./scripts/cstat_mmsc.sh" |tee -a ${LOG}
 #ssh omn@$h "rpm -qa | grep spot" |tee -a ${LOG}
